...
function checkIfLoggedIn(request, response, nextAction) {
  ...
  // if not logged in, send back to welcome
  else {
    request.session.destroy();
    response.sendFile(path.join(__dirname, "views", "welcome.html"));
  }
...
app.post(`/login`, async (request, response) => {
  const userData = await Users.checkUser(
...
app.post("/register", async (request, response) => {
  const success = await Users.addNewUser(
    request.body.username,
    request.body.password,
    request.body.boxID,
  );
...
app.get("/priorReadings", async (request, response) => {
  response.json({
    dataEntries: await Data.getDataFromDB(request.session.boxID),
  });
});
...
app.get("/mostRecentReading", async (request, response) => {
  response.json({
    dataEntriesOne: await Data.getOneDataFromDB(request.session.boxID),
  });
});
